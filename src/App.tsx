...
const WorldClock = ({ timezone, city }: { timezone: string; city: string }) => {
  ...
  const startAlarmSound = () => {
    ...
    const interval = setInterval(() => {
      if (!oscillatorRef.current) clearInterval(interval);
      playPulse();
    }, 1000);

    oscillatorRef.current = {} as any; // Dummy ref to keep track
  };
  ...
  const addAlarm = () => {
    if (!newAlarm) return;
    setAlarms(prev => [...prev, { id: Math.random().toString(), time: newAlarm, active: true }]);
    setNewAlarm("");
  };
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <motion.h2 
        ...
        className="text-7xl md:text-9xl font-bold text-white tracking-tighter mb-4 drop-shadow-xl tabular-nums"
      >
        {timeStr}
      </motion.h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
        ...
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 backdrop-blur-md">
          ...
          <div className="flex gap-2 mb-4">
            ...
            />
            <button onClick={addAlarm} className="bg-white/10 hover:bg-white/20 p-2 rounded-xl text-white transition-colors">
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 md:p-8 overflow-x-hidden relative">
      ...
      >
        ...
        <div className="relative group max-w-xl mx-auto">
          ...
          <AnimatePresence>
            {isSearchFocused && cities.length > 0 && (
              <motion.div
